...
## 出力形式（JSON・必須）
{
  ...
  "contentJa": "日本語解説記事本文（1000〜1500文字）。以下の構成で記述:\n1. 要約（150文字）：「[サービス名]とは何か」を一文で説明\n2. 詳細解説（600文字）：機能・特徴・海外での反応を具体的に説明\n3. 背景と文脈（200文字）：なぜ今この話題が注目されているか\n4. 日本への影響・なぜ今重要か（200文字）：日本ユーザーへの実用的な価値を説明\n5. [AFFILIATE_LINK]\n6. まとめと今後の展望（200文字）：読者が次に取るべきアクションを示す\n7. [AFFILIATE_LINK]\n8. 末尾に必ず「※この記事はAIが生成しています」を追記\n出典: ${originalUrl}",
...
- 元記事の事実を変えない
- 日本語として自然な表現を使う
- 本文は必ず1000文字以上1500文字以内で書く
- アフィリエイト商品への誘導文は含めない（別途挿入する）
- [AFFILIATE_LINK] プレースホルダーは本文中に2箇所挿入すること（セクション5と7）
- 本文末尾には必ず「※この記事はAIが生成しています」を追記すること
